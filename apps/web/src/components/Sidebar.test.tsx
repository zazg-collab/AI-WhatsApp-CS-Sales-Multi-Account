...
describe('Sidebar', () => {
  ...
  it('renders the always-visible nav links', () => {
    ...
    expect(screen.getByText('WA Accounts')).toBeInTheDocument();
  ...
  it('marks the active link based on pathname', () => {
    ...
    render(<Sidebar />);
    const active = screen.getByText('WA Accounts').closest('a');
    expect(active?.className).toContain('bg-indigo-50');
    const inactive = screen.getByText('Inbox').closest('a');
    expect(inactive?.className).not.toContain('bg-indigo-50');
  });
