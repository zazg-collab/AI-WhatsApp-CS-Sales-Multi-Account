...
import { dict } from './audit.i18n';

const ENTITY_OPTIONS = ['message','conversation','customer','bot','persona','account','campaign','knowledge_base','knowledge_item','user','settings','template','asset','product'] as const;

type RiskLevel = 'high' | 'medium' | 'low';

function classifyRisk(action: string): RiskLevel {
  const a = action.toLowerCase();
  if (a.includes('delete') || a.includes('hapus') || a.includes('ban') || a.includes('suspend') || a.includes('approve') || a.includes('campaign') || a.includes('password') || a.includes('role') || a.includes('takeover') || a.includes('pause_ai')) return 'high';
  if (a.includes('update') || a.includes('patch') || a.includes('restart') || a.includes('toggle') || a.includes('assign')) return 'medium';
  return 'low';
}
...
function DiffView({ oldValue, newValue }: { oldValue: unknown; newValue: unknown }) {
  const t = useT(dict);
  if (!oldValue && !newValue) return <span className="text-[11px] text-gray-400">{t('noChanges')}</span>;
  if (typeof oldValue === 'object' && oldValue !== null && typeof newValue === 'object' && newValue !== null && !Array.isArray(oldValue) && !Array.isArray(newValue)) {
    ...
    if (changed.length === 0) return <span className="text-[11px] text-gray-400">{t('noChanges')}</span>;
    return (
      <div className="space-y-1">
        {changed.map((key) => (
          <div key={key} className="flex flex-col gap-0.5 text-[11px]">
            <span className="font-medium text-gray-500">{key}</span>
            {key in oldObj && <span className="rounded bg-danger-50 px-1.5 py-0.5 font-mono text-danger-700 line-through dark:bg-danger-900/20 dark:text-danger-400">{String(oldObj[key] ?? '')}</span>}
            {key in newObj && <span className="rounded bg-channel-50 px-1.5 py-0.5 font-mono text-channel-700 dark:bg-channel-900/20 dark:text-channel-400">{String(newObj[key] ?? '')}</span>}
          </div>
        ))}
      </div>
    );
  }
  return <pre className="scrollbar-thin max-h-20 overflow-auto whitespace-pre-wrap rounded-md bg-gray-50 p-1.5 text-[11px] text-gray-500 dark:bg-gray-800 dark:text-gray-400">{JSON.stringify(newValue ?? oldValue, null, 2)}</pre>;
}
...
function CopyableId({ id }: { id: string }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <span title={id} className="max-w-[90px] truncate font-mono text-[11px] text-gray-400">{id.slice(0, 8)}…</span>
      <button onClick={copy} title={t('copyId')} className="shrink-0 rounded p-0.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-700">
        {copied ? <Check className="h-3 w-3 text-channel-600" aria-hidden="true" /> : <Copy className="h-3 w-3" aria-hidden="true" />}
      </button>
    </div>
  );
}
...
export default function AuditPage() {
  ...
  useEffect(() => { setPage(0); }, [entity, action, from, to]);
    ...
    <AppLayout>
      ...
      <div className="scrollbar-thin flex-1 overflow-y-auto p-5">
        <div className="mb-4 flex flex-wrap items-end gap-2">
          <div className="w-44"><SelectField label={t('entitas')} value={entity} onChange={(e) => setEntity(e.target.value)}><option value="">{t('semuaEntitas')}</option>{ENTITY_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}</SelectField></div>
          <div className="w-44"><Field label={t('aksi')} type="text" placeholder={t('cariAksi')} value={action} onChange={(e) => setAction(e.target.value)} /></div>
          <div className="w-36"><Field label={t('dariTanggal')} type="date" value={from} onChange={(e) => setFrom(e.target.value)} /></div>
          <div className="w-36"><Field label={t('sampaiTanggal')} type="date" value={to} onChange={(e) => setTo(e.target.value)} /></div>
          <div className="pb-0.5">
            <button onClick={() => setHighRiskOnly((v) => !v)} className={`flex items-center gap-1.5 rounded-md border px-3 py-[7px] text-[13px] font-medium transition-colors ${highRiskOnly ? 'border-danger-300 bg-danger-50 text-danger-700 dark:border-danger-700 dark:bg-danger-900/20 dark:text-danger-400' : 'border-gray-200 bg-white text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300'}`}>
              <Trash className="h-3.5 w-3.5" aria-hidden="true" />{t('showDestructive')}
            </button>
          </div>
        </div>
        ...
        {error && (
          <Card className="mb-4 flex items-start gap-2 border-danger-200 bg-danger-50 p-3 dark:border-danger-700/40 dark:bg-danger-900/20">
            ...
            <div>
              ...
              <button onClick={() => refetch()} className="mt-1 text-[13px] font-semibold text-danger-700 underline dark:text-danger-400">{t('cobaLagi')}</button>
            </div>
          </Card>
        )}
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="mt-4 flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => setPage(page - 1)} disabled={page === 0}><CaretLeft className="h-4 w-4" aria-hidden="true" />{t('sebelumnya')}</Button>
                <span className="text-xs text-gray-400">{t('pageInfo', { page: page + 1, total: totalPages, count: total })}</span>
                <Button variant="outline" size="sm" onClick={() => setPage(page + 1)} disabled={page >= totalPages - 1}>{t('berikutnya')}<CaretRight className="h-4 w-4" aria-hidden="true" /></Button>
              </div>
            )}
          </>
        )}
