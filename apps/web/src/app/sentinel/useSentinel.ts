...
import { dict } from './sentinel.i18n';
...
export function useSentinel() {
  ...
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem('sentinel_chat_history');
      if (stored) setChat(JSON.parse(stored));
    } catch { /* ignore */ }
  }, []);

  useEffect(() => {
    try { sessionStorage.setItem('sentinel_chat_history', JSON.stringify(chat)); }
    catch { /* ignore */ }
  }, [chat]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [a, r, s, g] = await Promise.all([
        api<Review[]>('/sentinel/alerts'),
        api<DailyReport>('/sentinel/reports/daily'),
        api<Snapshot>('/sentinel/snapshot'),
        api<KnowledgeGap[]>('/sentinel/knowledge-gaps'),
      ]);
  ...
  const loadInsight = useCallback(async (botId: string) => {
    ...
    setInsight(null);
    try {
      setInsight(await api<BotInsight>(`/sentinel/bot/${botId}/insight`));
    } catch {
      setInsight(null);
    } finally {
  ...
  useEffect(() => {
    ...
    return () => { socket.off('sentinel:alert', onAlert); };
  }, [load]);
  ...
  async function ask(e: React.FormEvent) {
    ...
    setAsking(true);
    try {
      const res = await api<{ answer: string }>('/sentinel/ask', {
        method: 'POST',
