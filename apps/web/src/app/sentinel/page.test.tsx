...
describe('SentinelPage', () => {
  ...
  it('renders alerts and daily report', async () => {
    apiMock.mockImplementation((path: string) => {
      ...
      if (path === '/sentinel/reports/daily')
  ...
  it('asks Sentinel a question', async () => {
    apiMock.mockImplementation((path: string) => {
      if (path === '/sentinel/ask') return Promise.resolve({ answer: 'Semua baik' });
      if (path === '/conversations/unread-count') return Promise.resolve({ count: 0 });
      if (path === '/sentinel/alerts') return Promise.resolve([]);
      return Promise.resolve(null);
    });
    render(<SentinelPage />);
    await waitFor(() => expect(apiMock).toHaveBeenCalledWith('/sentinel/alerts'));
    ...
    expect(apiMock).toHaveBeenCalledWith('/sentinel/ask', expect.objectContaining({ method: 'POST' }));
  });
});
