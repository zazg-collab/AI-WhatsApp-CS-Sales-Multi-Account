...
import {
  ...
  CheckCheck,
  Check,
  CheckCheck as CheckCheckDouble,
  UserPlus,
  PhoneCall,
  Loader,
} from 'lucide-react';
...
function InboxInner() {
  ...
  useEffect(() => {
    ...
    activePhoneRef.current = conv?.customer?.phoneNumber ?? null;
    if (!conv) setTypingPhone(null);
  }, [conv?.id, conv?.labels, conv?.customer?.phoneNumber]);
  ...
  useEffect(() => {
    ...
    const onEdited = ({ conversationId, messageId, content }: { conversationId: string; messageId: string; content: string }) =>
      setConv((prev) => (prev && prev.id === conversationId ? { ...prev, messages: prev.messages.map((m) => (m.id === messageId ? { ...m, content, editedAt: new Date().toISOString() } : m)) } : prev));
    const onDeleted = ({ conversationId, messageId }: { conversationId: string; messageId: string }) =>
      setConv((prev) => (prev && prev.id === conversationId ? { ...prev, messages: prev.messages.map((m) => (m.id === messageId ? { ...m, deletedAt: new Date().toISOString() } : m)) } : prev));
    const onReaction = ({ conversationId, messageId, reactions }: { conversationId: string; messageId: string; reactions: Record<string, string[]> }) =>
      setConv((prev) => (prev && prev.id === conversationId ? { ...prev, messages: prev.messages.map((m) => (m.id === messageId ? { ...m, reactions } : m)) } : prev));
    const onPresence = ({ phone, typing }: { phone: string; typing: boolean }) => {
      if (activePhoneRef.current === phone) {
        setTypingPhone(typing ? phone : null);
      }
    };
    const onConvUpdate = () => {
      loadList();
    ...
    const onCustomerAvatar = ({ customerId, avatarUrl }: { customerId: string; avatarUrl: string }) => {
      setConv(prev => prev?.customer?.id === customerId
        ? { ...prev, customer: { ...prev.customer, avatarUrl } }
        : prev);
      setList(prev => prev.map(c =>
        c.customer?.id === customerId
          ? { ...c, customer: { ...c.customer, avatarUrl } }
          : c
      ));
    };
    ...
    socket.on('customer:avatar', onCustomerAvatar);
    ...
    return () => {
      ...
      socket.off('customer:avatar', onCustomerAvatar);
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-gray-400">
                      ...
                      {typingPhone && (
                        <span className="ml-1 flex items-center gap-1 text-hermes-500">
                          <span className="flex gap-0.5">
                            <span className="h-1 w-1 animate-bounce rounded-full bg-hermes-500" style={{ animationDelay: '0ms' }} />
                            <span className="h-1 w-1 animate-bounce rounded-full bg-hermes-500" style={{ animationDelay: '150ms' }} />
                            <span className="h-1 w-1 animate-bounce rounded-full bg-hermes-500" style={{ animationDelay: '300ms' }} />
                          </span>
                          typing
                        </span>
                      )}
                    ...
                    <div key={m.id} className={cn('flex', isCustomer ? 'justify-start' : 'justify-end')}>
                      ...
                      >
                        ...
                        <div className={cn('mt-1 flex items-center justify-between gap-2 text-[10px] tabular-nums', isCustomer ? 'text-gray-400' : 'text-hermes-100')}>
                          ...
                          {!isCustomer && <MessageStatusIcon status={m.status} />}
                        </div>
                      </div>
                    </div>
                  );
...
function MediaContent({ message: m }: { message: Message }) {
  const resolved = resolveMediaUrl(m.mediaUrl);
  if (m.messageType === 'image') {
    if (resolved) {
      return <img src={resolved} alt={m.content ?? 'Image'} className="mb-1 max-h-60 rounded object-cover" />;
    }
    return (
  ...
  if (m.messageType === 'video') {
    if (resolved) {
      return <video src={resolved} controls className="mb-1 max-h-60 rounded" />;
    }
    return (
      <span className="flex items-center gap-1.5 italic opacity-80">
  ...
  if (m.messageType === 'audio') {
    if (resolved) {
      return <audio src={resolved} controls className="mb-1 w-full" />;
    }
    return (
      <span className="flex items-center gap-1.5 italic opacity-80">
  ...
  if (m.messageType === 'document') {
    if (resolved) {
      return (
        <a href={resolved} target="_blank" rel="noopener noreferrer" className="mb-1 flex items-center gap-1.5 underline opacity-80">
          <FileText className="h-4 w-4 shrink-0" strokeWidth={1.75} aria-hidden="true" />
          {m.content ?? 'Document'}
        </a>
      );
...
// WhatsApp-style message status indicator (checkmarks).
function MessageStatusIcon({ status }: { status: string }) {
  if (status === 'pending') return <Loader className="h-3 w-3 shrink-0 animate-spin" strokeWidth={2} />;
  if (status === 'sent') return <Check className="h-3 w-3 shrink-0" strokeWidth={2} />;
  if (status === 'delivered') return <CheckCheckDouble className="h-3 w-3 shrink-0" strokeWidth={2} />;
  if (status === 'read') return <CheckCheckDouble className="h-3 w-3 shrink-0 text-blue-300" strokeWidth={2} />;
  if (status === 'failed') return <CircleX className="h-3 w-3 shrink-0 text-red-300" strokeWidth={2} />;
  return null;
}
