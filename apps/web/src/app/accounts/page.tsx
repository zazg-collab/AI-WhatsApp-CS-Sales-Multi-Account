...
export default function AccountsPage() {
  ...
  async function deleteAccount(id: string, accountName: string) {
    if (!confirm(`Delete "${accountName}"? This will log out the WhatsApp session and remove all related data.`)) return;
    setDeleting(id);
    setError(null);
    try {
  ...
  async function logoutAccount(id: string, accountName: string) {
    if (!confirm(`Disconnect "${accountName}"? You can re-scan the QR to reconnect.`)) return;
    setLoggingOut(id);
    setError(null);
    try {
      await api(`/wa/accounts/${id}/logout`, { method: 'POST' });
      load();
    } catch (err) {
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {hasRole('supervisor') && (
                      <>
                        {!disconnected && (
                          <button
                            onClick={() => logoutAccount(a.id, a.accountName)}
                            disabled={loggingOut === a.id}
                            className="ml-2 rounded p-1.5 text-gray-400 transition-colors hover:bg-orange-50 hover:text-orange-600 dark:hover:bg-orange-900/20 dark:hover:text-orange-400"
                            title="Disconnect (logout)"
                          >
                            {loggingOut === a.id
                              ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-gray-300 border-t-orange-500" />
                        ...
                        <button
                          ...
                          className="ml-1 rounded p-1.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
