      ...
      {open && (
        <div className="mt-3 space-y-3 text-sm">
          ...
          <div className="flex flex-wrap gap-1">
            {DAY_LABELS.map((label, d) => (
              <button
                key={d}
                onClick={() => toggleDay(d)}
                className={`rounded px-2 py-1 text-xs ${days.includes(d) ? 'bg-wa-accent text-white' : 'bg-black/5 dark:bg-black/30 text-gray-600 dark:text-gray-400'}`}
              >
                {label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <button onClick={save} disabled={saving} className="rounded bg-wa-accent px-3 py-1 text-xs font-medium text-white disabled:opacity-50">
    ...
    <AppLayout><main className="mx-auto max-w-3xl p-8">
      ...
      <form onSubmit={addAccount} className="mb-8 flex gap-2">
        ...
        <input
          ...
          required
        />
        <button className="rounded bg-wa-accent px-4 font-medium text-white">
          Tambah
        </button>
      </form>
